...
const PasswordStrengthIndicator = ({ password, showRequirements = true }) => {
  ...
  const getStrengthLevel = () => {
    ...
    if (score < 2) return { level: 'Rất yếu', color: 'text-red-600 dark:text-red-400', bgColor: 'bg-red-100 dark:bg-red-900/20' };
    if (score < 3) return { level: 'Yếu', color: 'text-orange-600 dark:text-orange-400', bgColor: 'bg-orange-100 dark:bg-orange-900/20' };
    if (score < 4) return { level: 'Trung bình', color: 'text-yellow-600 dark:text-yellow-400', bgColor: 'bg-yellow-100 dark:bg-yellow-900/20' };
    if (score < 5) return { level: 'Mạnh', color: 'text-blue-600 dark:text-blue-400', bgColor: 'bg-blue-100 dark:bg-blue-900/20' };
    return { level: 'Rất mạnh', color: 'text-green-600 dark:text-green-400', bgColor: 'bg-green-100 dark:bg-green-900/20' };
  };
            ...
            return (
              ...
              >
                ...
                <span className={`text-sm transition-colors duration-200 ${
                  isValid ? 'text-green-700 dark:text-green-400' : 'text-gray-600 dark:text-gray-400'
                }`}>
                  {req.label}
                </span>
              </motion.div>
